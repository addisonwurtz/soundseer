...
from frequencycircle import FrequencyCircle
...
screen_w = int(infoObject.current_w / 1.1)
screen_h = int(infoObject.current_h / 1.1)

bass_band = FrequencyCircle(lower_bound=0,
                            upper_bound=300,
                            song_frequencies=bass_frequencies,
                            base_color=(255, 0, 0),
                            circle_x=screen_w * 0.5,
                            circle_y=screen_h * 0.5)
                            #circle_x= screen_w * 0.25,
                            #circle_y= screen_h * 0.25)

mid_band = FrequencyCircle(lower_bound=300,
                           upper_bound=4000,
...
treble_band = FrequencyCircle(lower_bound=4000,
                              upper_bound=8000,
                              ...
                              min_radius = 500,
                              max_radius = 500)
                              #circle_x=screen_w * 0.75,
                              #circle_y=screen_h * 0.75)
...
while running:
    ...
    for bass, mid, treble in zip(bass_band.bars, mid_band.bars, treble_band.bars):
        bass.update(deltaTime,
                    get_decibel(pygame.mixer.music.get_pos() / 1000.0, bass.freq, bass_band.frequency_index_ratio))
        mid.update(deltaTime,
                   get_decibel(pygame.mixer.music.get_pos() / 1000.0, mid.freq, mid_band.frequency_index_ratio))
        treble.update(deltaTime, get_decibel(pygame.mixer.music.get_pos() / 1000.0, treble.freq,
                                             treble_band.frequency_index_ratio))
